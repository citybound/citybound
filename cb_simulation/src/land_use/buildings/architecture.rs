...
use descartes::{N, P2, V2, Intersect, WithUniqueOrthogonal, LinePath,
ArcLinePath, ClosedLinePath, PrimitiveArea, Area};
use util::random::{Rng, seed};
use michelangelo::{Vertex, Mesh, Instance, Surface, FlatSurface,
Sculpture, SculptLine, SpannedSurface, SkeletonSpine};
    ...
    fn evaluate(&self, lot: &Lot) -> T {
        match *self {
            Choice::Random {
                ref options,
                ref ident,
            } => seed((lot.original_lot_id, ident))
                .choose(options)
...
impl FundamentRule {
    fn evaluate(&self, lot: &Lot) -> Result<SkeletonSpine, String> {
        ...
        SkeletonSpine::new(Rc::new(SculptLine::new(skeleton_path, 0.0)), width)
            .ok_or("Couldn't construct fundament skeleton spine".to_owned())
    }
}
...
fn family_house() -> BuildingRule {
    let windowed_facade_rule = FacadeRule::Face {
        ...
        decorations: vec![Choice::Specific(FacadeDecorationRule {
