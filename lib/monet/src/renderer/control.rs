...
pub fn setup(system: &mut ActorSystem) {
    system.extend::<Renderer, _>(|mut the_renderer| {
        ...
        the_renderer.on_critical(move |ctrl, renderer, world| match *ctrl {
                ...
                renderer.scenes.iter().enumerate() {
                                     ...
                                     Control::Render => {
                                         for (scene_id, mut scene) in
                renderer.scenes.iter_mut().enumerate() {
                                             for batch_to_clear in
                    (&mut scene)
                        .batches
                        .values_mut()
                        .filter(|batch| batch.clear_every_frame) {
                                     ...
                                     Control::Submit { target_ptr, return_to } => {
                                         let mut target =
                                             unsafe { Box::from_raw(target_ptr as *mut Frame) };
                                         ...
                                         world.send(return_to,
                                                    Submitted {
                                                        target_ptr: Box::into_raw(target) as usize,
                                                    });
