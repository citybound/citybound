...
pub fn old_smooth_path_from(points: &[P2]) -> Option<ArcLinePath> {
    ...
    let mut collected_path: Option<ArcLinePath> = None;
    ...
    for (end, start, direction) in end_start_directions {
        if let Some(valid_incoming_arc) = ArcLinePath::arc(previous_point, previous_direction, end)
        {
        ...
        if let Some(valid_connecting_line) = ArcLinePath::line(end, start) {
