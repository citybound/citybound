...
use actor_system::{LivingActor, ID, Known, SystemServices};
...
where S : Known {
    ...
    pub fn create(&mut self, state: S) -> LivingActor<S> {
        LivingActor {
            id: self.allocate_id(),
            state: state
        }
    ...
    pub fn at(&self, id: usize) -> &LivingActor<S> {
        let i = self.slot_map.indices_of(id);
        unsafe {
            let actor : &LivingActor<S> = transmute(
    ...
    pub fn at_mut(&mut self, id: usize) -> &mut LivingActor<S> {
        let i = self.slot_map.indices_of(id);
        unsafe {
            let actor : &mut LivingActor<S> = transmute(
    ...
    pub fn add(&mut self, actor: &LivingActor<S>) {
        ...
        unsafe {
            let actor_in_slot : &mut LivingActor<S> = transmute(ptr);
    ...
    fn swap_remove(&mut self, indices: SlotIndices) {
        unsafe {
            ...
            let swapped_actor : &LivingActor<S> = transmute(collection.swap_remove(indices.slot()));
    ...
    pub fn resize(&mut self, id: usize) {
            ...
            let old_actor_ptr = self.at(id) as *const LivingActor<S>;
            unsafe {
    ...
    pub fn receive<M: Message>(&mut self, id: usize, message: &M, system: &mut SystemServices) where S: Recipient<M> {
