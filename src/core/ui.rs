    ...
    fn receive(&mut self, msg: &Mouse) -> Fate {
        match *msg {
            Mouse::Moved(position) => {
                ...
                if self.space_pressed {
                    if self.shift_pressed {
                        ...
                        MoveEye {
                            scene_id: 0,
                            movement: ::monet::Movement::Rotate(-delta.x / 300.0),
                        };
                        Renderer::id() <<
                        MoveEye {
                            scene_id: 0,
                            movement: ::monet::Movement::Zoom(-delta.y),
                        };
                        ...
                        MoveEye {
                            scene_id: 0,
                            movement: ::monet::Movement::Shift(V3::new(-delta.y / 3.0,
                                                                       delta.x / 3.0,
                                                                       0.0)),
                        };
                        self.cursor_2d = position;
            ...
            Mouse::Scrolled(delta) => {
                ...
                MoveEye {
                    scene_id: 0,
                    movement: ::monet::Movement::Shift(V3::new(delta.y / 5.0, -delta.x / 5.0, 0.0)),
                };
                Fate::Live
            }
            Mouse::Down => {
        ...
        match *msg {
            Key::Down(key_code) => {
                ...
                self.focused_interactable
                    .map(|interactable| interactable << Event3d::KeyDown(key_code));
                Fate::Live
            }
            Key::Up(key_code) => {
                match key_code {
                ...
                self.focused_interactable
                    .map(|interactable| interactable << Event3d::KeyUp(key_code));
                Fate::Live
            }
        }
    }
...
pub fn process_events(window: &GlutinFacade) -> bool {
    ...
    if let Some(last_mouse_move_event_pos) =
        cleaned_events.iter().rposition(|event| match *event {
