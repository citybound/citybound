...
impl GridAccelerator {
        ...
        for bbox in bboxes {
            ...
            for cell_x in x_start..x_end {
                for cell_y in y_start..y_end {
                    let cell = self.cells.entry((cell_x, cell_y)).or_insert_with(RoaringBitmap::new);
    ...
    pub fn colocated_for(&self, query_ref: usize) -> Vec<usize> {
        self.cells_of.get(&query_ref).into_iter().flat_map(|coordinates_set|
            coordinates_set.iter().flat_map(|coordinates|
                self.cells[coordinates].iter().map(|ref_u64| ref_u64 as usize)
            )
        ).collect::<FnvHashSet<_>>().into_iter().filter(|found_ref| found_ref != &query_ref).collect::<Vec<_>>()
    }
