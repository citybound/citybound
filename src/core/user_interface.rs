...
impl UserInterface {
    fn new() -> UserInterface {
        UserInterface {
            ...
            zoom_modifier: Some(Settings::register_mouse(KeyCombination { keys: vec![] })),
            mouse_modifier: Some(Settings::register_mouse(KeyCombination { keys: vec![] })),
            ...
            Projected3d { position_3d } => {
                ...
                }
                {
                    ...
                    if self.hovered_interactable != new_hovered_interactable {
                        ...
                        Action::Event3d(Event3d::HoverOngoing { at: self.cursor_3d });
                    }
...
impl Recipient<Action> for UserInterface {
    fn receive(&mut self, msg: &Action) -> Fate {
        println!("{:?}", msg);
        if let Some(focused_interactable) = self.focused_interactable {
            focused_interactable << *msg
        }
        match *msg {
            Action::Mouse(MouseAction { action_id: id, mouse: Mouse::Moved(position) }) => {
                ...
                println!("{}, {}, {}",
                         self.mouse_modifier.unwrap(),
                         self.yaw_modifier.unwrap(),
                         self.pitch_modifier.unwrap());

                if Some(id) == self.yaw_modifier {
                    Renderer::id() <<
                    MoveEye {
                        scene_id: 0,
                        movement: Yaw(-delta.x * Settings::get_rotation_speed() *
                                      Settings::get_invert_y() /
                                      300.0),
                    };
                }
                if Some(id) == self.pitch_modifier {
                    ...
                    MoveEye {
                        scene_id: 0,
                        movement: Pitch(-delta.y * Settings::get_rotation_speed() *
                                        Settings::get_invert_y() /
                                        300.0),
                    };
                }
                if Some(id) == self.pan_modifier {
                    ...
                    MoveEye {
                        scene_id: 0,
                        movement: Shift(V3::new(-delta.y * Settings::get_move_speed() *
                                                Settings::get_invert_y() /
                                                3.0,
                                                delta.x * Settings::get_move_speed() *
                                                Settings::get_invert_y() /
                                                3.0,
                                                0.0)),
                    };
            ...
            Action::KeyHeld(KeyAction { action_id: id }) => {
                if Some(id) == self.forward_action_id {
                    ...
                    MoveEye {
                        scene_id: 0,
                        movement: ::monet::Movement::Shift(V3::new(5.0 *
                                                                   Settings::get_move_speed(),
                                                                   0.0,
                                                                   0.0)),
                    };
                }
            ...
            Action::KeyDown(KeyAction { action_id: id }) => {
                if Some(id) == self.mouse_drag {
                    ...
                    if let Some(active_interactable) = self.active_interactable {
                        active_interactable <<
                        Action::Event3d(Event3d::DragStarted { at: self.cursor_3d });
                    }
                }
            }
