...
use super::plan::{PlanDelta, PlanResultDelta, RemainingOldStrokes, LaneStrokeRef};
...
#[derive(Copy, Clone)]
pub struct WithLatestNode(P2, bool);
...
#[derive(Copy, Clone)]
pub struct Select(SelectableStrokeRef, N, N);
...
#[derive(Copy, Clone)]
pub struct SetSelectionMode(bool, bool);
...
impl CurrentPlan {
    fn create_selectables(&mut self) {
        for (stroke_idx, stroke) in self.preview.delta.new_strokes.iter().enumerate() {
            Swarm::<Selectable>::all() <<
            CreateWith(Selectable::new(SelectableStrokeRef::New(stroke_idx),
                                       stroke.path().clone()),
                       AddToUI);
        }
        for (old_ref, stroke) in self.preview.current_remaining_old_strokes.mapping.pairs() {
            Swarm::<Selectable>::all() <<
            CreateWith(Selectable::new(SelectableStrokeRef::RemainingOld(*old_ref),
                                       stroke.path().clone()),
                       AddToUI);
        }
    }
        ...
        if let DrawingStatus::WithSelections(ref selections, _) =
            ...
            for (&selection_ref, &(start, end)) in selections.pairs() {
                ...
                if let Some(subsection) = stroke.path().subsection(start, end) {
                    Swarm::<Draggable>::all() <<
                    CreateWith(Draggable::new(selection_ref, subsection), AddToUI);
                }
            }
        }
    }
            ...
            for (&selection_ref, &(start, end)) in selections.pairs() {
                ...
                if !all_strokes.any(is_right_of_stroke) {
                    if let Some(shifted_stroke) =
                        ...
                        Swarm::<Addable>::all() <<
                        CreateWith(Addable::new(shifted_stroke), AddToUI);
                    }
                }
            }
        }
    ...
    fn clear_selectables(&mut self) {
        Swarm::<Selectable>::all() << ClearSelectables;
    }

    fn clear_draggables(&mut self) {
        Swarm::<Draggable>::all() << ClearDraggables;
        Swarm::<Addable>::all() << ClearDraggables;
    }
}
...
#[derive(Copy, Clone)]
pub struct ClearSelectables;

#[derive(Copy, Clone)]
pub struct ClearDraggables;

#[derive(Copy, Clone)]
pub struct ClearAddables;
