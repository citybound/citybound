    ...
    fn receive(&mut self, msg: &Simulate) -> Fate {match *msg{
        Simulate{requester, ref delta} => {
            let state = match *self{
                Ready(ref state) | WaitingForUnbuild(_, _, ref state, _, _, _) => state
            };
            ...
            *self = match *self {
                ...
                Ready(ref mut state) => {
                    ...
                    for old_ref in result_delta.trimmed_strokes.to_destroy.keys() {
                        let id = state.built_trimmed_lanes.remove(*old_ref).expect("tried to unbuild a non-existing lane");
                        ids_to_unbuild.push(id);
                    }

                    for old_ref in result_delta.transfer_strokes.to_destroy.keys() {
                        let id = state.built_transfer_lanes.remove(*old_ref).expect("tried to unbuild a non-existing transfer lane");
                        ids_to_unbuild.push(id);
                    }
                    ...
                    WaitingForUnbuild(requester, ids_to_unbuild, state.clone(), new_plan, new_result, result_delta)
                }
            };
            Fate::Live
        }
...
impl Recipient<ReportLaneBuilt> for MaterializedReality {
    fn receive(&mut self, msg: &ReportLaneBuilt) -> Fate {match *msg{
        ReportLaneBuilt(id, buildable_ref) => {
            match *self {
                Ready(ref mut state) => match buildable_ref {
                    BuildableRef::Intersection(index) => {
                        if let Some(other_intersection_lanes) = state.built_intersection_lanes.get(IntersectionRef(index)) {
                    ...
                    BuildableRef::TrimmedStroke(index) => {state.built_trimmed_lanes.insert(TrimmedStrokeRef(index), id);},
                    BuildableRef::TransferStroke(index) => {state.built_transfer_lanes.insert(TransferStrokeRef(index), id);}
                },
                WaitingForUnbuild(..) => panic!("a waiting materialized reality shouldn't get build reports")
            }
            Fate::Live
        }
    }}
...
impl Recipient<ReportLaneUnbuilt> for MaterializedReality {
    fn receive(&mut self, msg: &ReportLaneUnbuilt) -> Fate {match *msg{
        ReportLaneUnbuilt(maybe_id) => {
            let maybe_new_self = match *self {
                WaitingForUnbuild(requester, ref mut ids_to_unbuild, ref state, ref new_plan, ref new_result, ref result_delta) => {
                    if let Some(id) = maybe_id {
                        let pos = ids_to_unbuild.iter().position(|unbuild_id| *unbuild_id == id).expect("Trying to delete unexpected id");
